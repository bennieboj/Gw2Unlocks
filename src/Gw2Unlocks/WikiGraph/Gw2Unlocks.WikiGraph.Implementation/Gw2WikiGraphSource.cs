...
public sealed class Gw2WikiGraphSource(
    ILogger<Gw2WikiGraphSource> logger,
    IGw2WikiSource wikiCache) : IGw2WikiGraphSource
{
    public async Task<AcquisitionGraph> GetAcquisitionGraph(CancellationToken cancellationToken = default)
    {
            ...
            try
            {
                foreach (var (title, text) in ExtractPages([xml]))
                {
                    try
                    {
                        ...
                        var infobox = ParseInfobox(title, ast);
                        if (infobox == null)
                            continue;
    ...
    private static InfoboxData? ParseInfobox(string title, Wikitext ast)
    {
        ...
        if (info.Get("type")?.Equals("Container", StringComparison.OrdinalIgnoreCase) == true)
        {
            ...
            foreach (var template in ast.EnumDescendants().OfType<Template>())
            {
                ...
                if (!string.IsNullOrWhiteSpace(target))
                {
                    ...
                    graph.AddEdge(nodeId, target, EdgeType.Contains);
                }
            }
        }
